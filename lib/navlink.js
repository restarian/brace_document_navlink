		...
		// the data object as well.
			...
			data_keys.forEach((location, index, proto) => {
				...
				if ( !found_link ) {
					...
					page.content = page.content.replace(RegExp(heading), (heading==="^"&&EOL||heading) + 
						EOL+"---"+EOL+"### " + nav_title + EOL+nav_list.join(EOL)+EOL+EOL+"---"+EOL)
				...
				page.content = page.content.replace(header_regex, nav_markup + nav_title + EOL + nav_list.join(EOL) + EOL+EOL)
