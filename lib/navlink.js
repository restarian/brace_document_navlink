	...
	def.prototype = { 
		...
		findUrl: function(dir, cb, err) {
			...
			utils.Spawn("git", ["config", "--get", "remote.origin.url"], {cwd: dir}, (code, stdout, stderr) => { 
				...
				utils.Spawn("git", ["rev-parse", "--abbrev-ref", "HEAD"], (code, stdout, stderr) => {

					if ( stderr || !stdout ) 
						return err_cb(up_err.log("The branch name url for this project is not available and will not set."))
		...
		createDocument: function(structure, page_data, navlink_url, backup_dir, cb, err) {
			...
			// Set the chaining logging titles to also have this member name.
			var up_err = this.up_err.s(), up = this.up.s()
			up.log_title = up.log_title + " - createDocument()"
			up_err.log_title = up.log_title + " - ERROR"
