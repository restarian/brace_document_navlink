...
define(["bracket_print", "bracket_utils"], function(print, utils) {
	...
	def.prototype = { 
		...
		// the data object as well.
			...
			var header_regex_text = "(["+EOL+"]+[\\-,\\*\\_]{3,}["+EOL+"]+#+[\\ ,\\t]+)([^"+EOL+"]+)["+EOL+"]"
			...
			var bullet_regex = RegExp(header_regex_text + "(\\s*[\\*,\\+,\\-]\\s+.*["+EOL+"])")
			...
			data_keys.forEach((location, index, proto) => {
				...
				var nav_title = this.option.forceTitle && this.option.title || (navlink_denotation[2] || this.option.title)
				...
				if ( !found_bullet ) {
					...
					while ( found_bullet ) {
						...
						found_bullet = false
						// The loop is continued if another bullet is found under the navigation sub-heading. The loop will stop when all of the bullets 
						// that are directly under the navigation heading are removed from the contents string.
						page.content = page.content.replace(bullet_regex, function(all, heading_identifier, heading_text, navlink_bullet) { 
							// Set the loop to go again :and return the first match to be matched again. 
							if ( RegExp("^["+EOL+"]+.*").test(navlink_bullet) ) 
								return (found_bullet = false) || all 

							return found_bullet = true && (heading_identifier + navlink_bullet) 
		...
		// Turn a string into words by converting the non-letter characters into spaces and capitalizing the first letter of the first word.
			...
			.replace(RegExp("["+EOL+",\t]", "g"), "").replace(/\W+/g, " ").replace(/[\_]+/g, " ")
